...
def compute_adv_loss(p, targets, model, domain_attack=False, domain='loc'):  # predictions, targets, model
    ...
    # Define criteria
    BCEcls = nn.BCEWithLogitsLoss(pos_weight=ft([1.]))
    BCEobj = nn.BCEWithLogitsLoss(pos_weight=ft([1.]))
...
def plot_results(start=0, stop=0):  # from utils.utils import *; plot_results()
    ...
    for f in sorted(glob.glob('results*.txt') + glob.glob('weights/results*.txt')):
    ...
    fig.tight_layout()
    ax[1].legend()
    fig.savefig('weights/results.png', dpi=200)

def plot_results_adv(start=0, stop=0):  # from utils.utils import *; plot_results()
    # Plot training results files 'results*.txt'
    fig, ax = plt.subplots(2, 3, figsize=(14, 7))
    ax = ax.ravel()
    s = ['Total_loss', 'Admm_loss','Train_loss', 'Thre', 'mAP','mAP_adv']
    for f in sorted(glob.glob('results*.txt') + glob.glob('weights/results*.txt')):
        results = np.loadtxt(f, usecols=[5, 6, 7, 14, 10, 11], ndmin=2).T
        n = results.shape[1]  # number of rows
        x = range(start, min(stop, n) if stop else n)
        for i in range(6):
            y = results[i, x]
...
def get_test_input(img, img_shape, input_shape = (416,416)):
    ih, iw = img_shape
    h, w = input_shape
    scale = min(w/iw, h/ih)
    nw = int(iw*scale)
    ...
    img_temp = new_image[:,:,::-1].transpose((2, 0, 1)).copy()
    img_temp = img_temp[np.newaxis,:]
    return img_temp


def img_convert_inv(img, img_shape, input_shape=(416,416)):
    img = img.squeeze()
...
def preprocess_true_boxes(true_boxes, input_shape, anchors, num_classes, img_shape):
    ...
    dx = (w - nw) // 2
    dy = (h - nh) // 2
    true_boxes[..., [0, 2]] = true_boxes[..., [0, 2]] * scale + dx
    true_boxes[..., [1, 3]] = true_boxes[..., [1, 3]] * scale + dy
...
def check_distortion(perts_np):
    distortion = np.square(perts_np)
    mean_distort = np.mean(distortion, axis=(1, 2, 3))
    return mean_distort