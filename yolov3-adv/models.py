...
class Darknet(nn.Module):
    ...
    def __init__(self, cfg, img_size=(416, 416), arc='default', mlp=False):
        ...
        self.hyp = hyp
        self.nc = self.module_list[self.yolo_layers[0]].nc
        self.arc = arc
        ...
        if mlp:
            dim_mlp = 1024
            self.mlp = nn.Sequential(nn.Linear(dim_mlp, dim_mlp), nn.ReLU(), nn.Linear(dim_mlp, dim_mlp))


    def forward(self, x, var=None, fa=False, only_fa=False):
        img_size = x.shape[-2:]
        layer_outputs = []
        output = []
        for i, (mdef, module) in enumerate(zip(self.module_defs, self.module_list)):
            ...
            # darknet53 the layer before avgpool
            if i == 74 and fa:
        ...
        if fa and self.training:
            return output, avg_out
        if fa and not self.training:
            io, p = list(zip(*output))  # inference output, training output
            return torch.cat(io, 1), p, avg_out
        if self.training:
            return output
        elif ONNX_EXPORT:
...
def convert(cfg='cfg/yolov3-spp.cfg', weights='weights/yolov3-spp.weights', mlp=False):
    # Converts between PyTorch and Darknet format per extension (i.e. *.weights convert to *.pt and vice versa)
    # from models import *; convert('cfg/yolov3-spp.cfg', 'weights/yolov3-spp.weights')

    # Initialize model
    model = Darknet(cfg, mlp=mlp)

    # Load weights and save
    if weights.endswith('.pt'):  # if PyTorch format
        model.load_state_dict(torch.load(weights, map_location='cpu')['model'])
        save_weights(model, path=weights.replace('pt','weights'), cutoff=-1)
