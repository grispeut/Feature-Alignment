...
def generate_anchors_single_pyramid(scales, ratios, shape, feature_stride, anchor_stride, rpn_rank_inds):
    """
    ...
    if boxes.shape[0] > rpn_rank_inds.shape[0]:
        boxes = boxes[rpn_rank_inds.cpu().numpy(),:]
    return boxes


def generate_anchors_all_pyramids(scales, ratios, feature_shapes, feature_strides,
                             anchor_stride, rpn_rank_inds):
    ...
    for i in range(len(scales)):
        anchors.append(generate_anchors_single_pyramid(scales[i], ratios, feature_shapes[i],
                                        feature_strides[i], anchor_stride, rpn_rank_inds[i]))
