...
class _ProposalTargetLayer(nn.Module):
    ...
    def _get_bbox_regression_labels_pytorch(self, bbox_target_data, labels_batch, num_classes):
        ...
        for b in range(batch_size):
            # assert clss[b].sum() > 0
            if (clss[b]-num_classes+1).sum() == 0:
                continue
            inds = torch.nonzero(clss[b] < (num_classes-1), as_tuple=False).view(-1)
    ...
    def _sample_rois_pytorch(self, all_rois, gt_boxes, fg_rois_per_image, rois_per_image, num_classes):
        ...
        labels_batch = labels.new(batch_size, rois_per_image).zero_() + (num_classes - 1)
        ...
        for i in range(batch_size):
            ...
            labels_batch[i][fg_rois_per_this_image:] = (num_classes - 1)
